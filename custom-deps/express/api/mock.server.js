...
const argRe = /^\-{1,2}([a-z]+?)=(.*)$/;
const argsMap = process.argv.splice(2).reduce((map, argStr) => {
...
let _existRoutes = [];
const isSavedURLMatch = (m, p) => {
  const tmpStr = `${m.toUpperCase()} ${p}`;
  if (~_existRoutes.indexOf(tmpStr)) return true;
  return _existRoutes.some((r) => {
    // look up for route like "/foo/:id"
...
app.use((req, res, next) => {
  if (!isSavedURLMatch(req.method, req.path)) {
    let url = req.originalUrl;
    let target = "";
    for (let i = 0; i < proxy.length; i++) {
      const p = proxy[i];
      if (
        url.substr(0, p.prefix.length) === p.prefix &&
        p.target.length > target.length
      ) {
        // '/api/v1' rather than '/api'
        target = p.target;
      }
    }
...
walk(__dirname)
  ...
  .map((p) => p.path)
  .forEach((part) => require(part)(app, config.prefix));

_existRoutes = app._router.stack
  .filter((s) => s.route)
  .map((s) => {
    const { path, stack } = s.route;
    let m = "";
    try {
      m = `${stack[0].method.toUpperCase()} `;
    } catch (ex) {
      console.log(ex);
    }
    return `${m}${path}`;
